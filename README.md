...
### Beyond bar plots — beeswarm, dependence, and a cross-check

[Notebook 07](notebooks/07_feature_importance.ipynb) also covers three additional explainability views, all rendered inline:

- **Beeswarm summary** — every test cycle as a dot; colour = feature value, X = SHAP contribution. Reveals direction *and* spread per feature, not just rank.
- **Dependence plots** for the top 3 sensors — does the model respond to a smooth threshold or a step function? Auto-detected interaction features add a second dimension via colour.
- **Permutation importance cross-check** — model-agnostic ranking compared side-by-side with the SHAP ranking. When two independent attribution methods agree on the top sensors, that's a credibility signal; when they disagree, the gap is informative (e.g. correlated features rank high on SHAP but low on permutation).

The Streamlit dashboard's Global Feature Importance panel exposes the beeswarm and dependence plots interactively (toggle "Compute global feature importance" → expand the new collapsible panels).
...
- **No SHAP interaction values.** The dependence plots auto-select an interaction feature heuristically; computing the full pairwise interaction matrix (`shap_interaction_values`) would let the dashboard surface explicit "sensor A only matters when sensor B is in regime X" findings — useful but compute-heavy.
...
| **Explainability** | SHAP (TreeExplainer, dependence + beeswarm), Permutation Importance (cross-check) |
